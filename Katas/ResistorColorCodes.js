...
For resistors less than 1000 ohms, return a string containing the number of ohms, a space, the word "ohms" followed
by a comma and a space, the tolerance value (5, 10, or 20), and a percent sign. For example, for the "yellow violet black"
...
function decodeResistorColors(bands) {
    ...
    let bandsArray = bands.split(' ')

    let resistance = (ColorCodes.indexOf(bandsArray[0]) * 10 + ColorCodes.indexOf(bandsArray[1])) * (10 ** ColorCodes.indexOf(bandsArray[2]))
    if (resistance < 1000) {
        resistance = resistance.toString() + ' ohms'
    } else if (resistance < 1000000) {
        resistance = (resistance / 1000).toString() + 'k ohms'
    } else {
        resistance = (resistance / 1000000).toString() + 'M ohms'
    }
    let tolerance = ''
    if (bandsArray.length === 3) {
        tolerance = '20%'
    } else {
        if (bandsArray[3] === 'gold')
            tolerance = '5%'
        else {
            tolerance = '10%'
        }
    }
    return resistance.toString() + ', ' + tolerance
}
